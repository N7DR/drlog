// $Id: rules.cpp 299 2026-07-26 20:18:51Z  $
...
*/
void exchange_field_values::add_canonical_value(const string_view cv)
{ if (!(_values.contains(cv)))
  { string cvs { cv };

    _values += { cvs, STRING_SET { cvs } };
  }
}
...
//      qthx.name("QTHX["s + canonical_prefix + RIGHT_SQUARE_BRACKET);
      qthx.name("QTHX"s + delimit(canonical_prefix, SQUARE_BRACKETS));
...
// the next line can throw an exception if rig has been set to a mode that is not supported by the contest
  ...
  { ost << "Out of Range error in contest_rules::_exchange_fields: " << oor.what() << endl;
    ost << "canonical prefix = " << canonical_prefix << ", mode = " << ::to_string(m) << ", expand_choices = " << ((expand_choices == CHOICES::EXPAND) ? "true" : "false") << endl;
...
// put into alphabetical order
    ...
    if (is_opt)
    { try
      { const string_view name { split_string <string_view> (field_name, COLON).at(1) };
...
//  _original_score_bands = SR::to<flat_set>(_score_bands);
...
// generate the country mults; the value from context is either "ALL" or "NONE" or a comma-separated list
  ...
  { if (context.country_mults_filter() == "ALL"sv)
      ...
      FOR_ALL(clean_split_string <string> (context.country_mults_filter()), [this] (const string& prefix) { _country_mults += prefix; } );
  }
...
// remove any country mults that are explicitly not allowed
  FOR_ALL(clean_split_string <string> (context.not_country_mults()), [this, &location_db] (const string& not_country_mult) { _country_mults -= location_db.canonical_prefix(not_country_mult); } );
...
// add the permitted bands
  ...
  { try
    ...
    { ost << "Unknown band in config file: " << str << endl; }
  }

  if (_permitted_bands.empty())
  { ost << "ERROR: no permitted bands" << endl;
    exit(-1);
  }
...
// generate the sets of all acceptable values
  FOR_ALL(all_known_field_names(), [this] (const string& field_name) { _permitted_exchange_values += { field_name, _all_exchange_values <STRING_SET> (field_name) }; });
...
*/
STRING_SET contest_rules::all_known_field_names(void) const
{ STRING_SET rv;
...
{ SAFELOCK(rules);
  ...
  return MUM_VALUE(_exchange_field_eft, field_name, EFT("none"sv));
}
...
{ if (actual_value.empty())
  ...
  if ((field_name == "IOTA"sv) and (actual_value.length() > 2))   // IOTA is special because there are so many possible received values, many of which are not canonical
    return (substring <string> (actual_value, 0, 2) + pad_leftz(substring <string_view> (actual_value, 2), 3));  // XXnnn
...
*/
  ...
  return MUM_VALUE(_permitted_to_canonical.at(field_name), def_value, def_value);  // we don't get here if field_name isn't valid
}
...
// is an exchange field that will determine the number of points present?
  ...
  const auto& pb { _points[to_uint(m)] };          // select the correct mode

  SAFELOCK(rules);

  if (!pb.contains(b))
  { ost << "Unable to find any points entries for band " << ::to_string(b) << endl;
    return 0;
  }
...
*/
bool contest_rules::sent_exchange_includes(const string_view str, const MODE m) const
{ SAFELOCK(rules);
...
STRING_SET contest_rules::exchange_field_names(void) const
{ STRING_SET rv;

  for (const auto& [ field_name, _ ] : _exchange_field_eft)      //std::map<std::string /* field name */, EFT>   _exchange_field_eft;
    rv += field_name;
...
{ choice_equivalents rv { };
  ...
  STRING_MAP<choice_equivalents>::const_iterator cit_cp { choice_equivalents_this_mode.find(cp) };
...
{ if (field_name == "DOK"sv)
  { if (!received_value.empty())
    { const auto posn { received_value.find_first_of(UPPER_CASE_LETTERS) };
      ...
      return string { received_value };  // same as string()
  }
