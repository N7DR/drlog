// $Id: drlog.cpp 299 2026-07-26 20:18:51Z  $
...
//import <chrono>;  // import not yet supported
...
//import std;  // import not yet supported
...
using enum WINDOW_ATTRIBUTES;   // save some typing
...
time_log <std::chrono::milliseconds> global_timer;        // so that we have access to how long we've been running, if we need it

// write the value of the global timer (in milliseconds) to the logfile
inline void log_timer(const string_view str)
  { ost << str << " at time: " << css(static_cast<int>(global_timer.click())) << "ms" << endl; }

// needed for WRAPPER_3 definition of memory_entry
inline ostream& operator<<(ostream& ost, const DRLOG_MODE& dm)
{ return (ost << ( (dm == DRLOG_MODE::CQ) ? 'C' : 'S' ));
...
char   TS { COMMA };      ///< character for thousands separator
...
string build_rit_xit_str(const polled_status& status);                              ///< Build the rit_xit_str string to be displayed
...
pt_mutex            thread_check_mutex { "THREAD CWINDOW_ATTRIBUTES::HECK"s };   ///< mutex for controlling threads; both the following variables are under this mutex
...
unsigned int            bandscope_span_cq { 5 };                   ///< span of P3 when in CQ mode, in kHz
unsigned int            bandscope_span_sap { 20 };                 ///< span of P3 when in SAP mode, in kHz
...
rig_capabilities        rcaps { };                          ///< rig capabilities
...
frequency               ssb_bandwidth_narrow { 1600_Hz };   ///< narrow SSB bandwidth
//int                     ssb_bandwidth_wide   { 1800 };      ///< wide SSB bandwidth, in Hz
frequency               ssb_bandwidth_wide   { 1800_Hz };   ///< narrow SSB bandwidth
...
constinit int           type_1_post_counter { 0 };          ///< counter for type 1 posts
constinit int           type_2_post_counter { 0 };          ///< counter for type 1 posts
...
*/
bool ok_to_poll_rig { true };                  ///< is it reasonably safe to poll the rig?
...
rig_interface*    rig_ptr { nullptr };      ///< new rig control (includes dynamic polymorphism)
...
inline void update_recording_status_window(void)
  { win_recording_status < WINDOW_CLEAR < CURSOR_START_OF_LINE <= ( (allow_audio_recording and audio.recording()) ? "REC"s : "---"s ); }
...
// generate version information
  ...
  { const FLAT_STRING_MAP<string> MONTH_NAME_TO_NUMBER( { { "Jan"s, "01"s },
    ...
    const string date_str { DATE_STR.substr(DATE_STR.length() - 4) + DASH + MONTH_NAME_TO_NUMBER.at(DATE_STR.substr(0, 3)) + DASH +
                            (DATE_STR[4] == SPACE ? "0"s + DATE_STR.substr(5, 1) : DATE_STR.substr(4, 2)) };

    VERSION = VERSION_TYPE + SPACE + date_str + SPACE + TIME_STR.substr(0, 5);
...
// configure table for checking connectivity to other machines
    ...
    bandscope_span_cq               = context.bandscope_span_cq();
    bandscope_span_sap              = context.bandscope_span_sap();
    ...
    if (const string fn { context.rig_capabilities_filename() }; !fn.empty())
    { if (const string actual_name { find_file(context_path, fn) }; actual_name.empty())
      { ost << "Unable to read rig capabilities file: " << fn << endl;
        exit(-1);
      }
      else
    ...
    ssb_bandwidth_narrow            = frequency(context.ssb_bandwidth_narrow());
    ssb_bandwidth_wide              = frequency(context.ssb_bandwidth_wide());
...
// create the set of blocked canonical prefixes
    dx_post::blocked_posts = SR::to<FLAT_STRING_SET>( context.blocked_posts() | SRV::transform([] (const string& blocked_post) { return location_db.canonical_prefix(blocked_post); }) );

    if (!dx_post::blocked_posts.empty())
      ost << "blocked canonical prefixes: " << join(dx_post::blocked_posts, ", "sv) << endl;
...
// MESSAGE window (do this as early as is reasonable so that it's available for messages)
    ...
    win_message < WINDOW_BOLD <= EMPTY_STR;                                       // use bold in this window
...
// possibly open communication with the rig
      ...
      { try
        {
// can't use strings in switch statements :-( We could fake it, but it's not worth it unless/until we have several special rigs
          ...
          if (!rig_ptr)
          { rig_ptr = new rig_interface;
            ost << "rig interface set to default" << endl;
          }

          rig_ptr -> prepare(context);        // set up communication; define rig capabilities

// explicit capabilities override the generic ones for the rig
          if (!rcaps.empty())       // only non-empty if caps have been read from a file
            rig_ptr -> rcaps(rcaps);
...
// the rig might have changed mode if we just changed bands
      if (current_mode != rig_ptr -> rig_mode())
        rig_ptr -> rig_mode(current_mode);

      if (current_mode == MODE_CW)
        rig_ptr -> bandwidth(cw_bandwidth_wide);   // set to default SAP bandwidth if on CW

      rig_ptr -> base_state();       // put rig in known base state
...
// now the individual bands
        for (BAND b { MIN_BAND }; b <= MAX_BAND; b = (BAND)((int)b + 1))
          FOR_ALL(calls_from_do_not_show_file(b), [&bm = bandmaps[static_cast<unsigned int>(b)]] (const auto& callsign) { bm.do_not_add(callsign); });
      }
...
// add my marker to each bandmap
      ...
      { bandmap&      bm { bandmaps[static_cast<unsigned int>(b)] };
        bandmap_entry be { my_bandmap_entry };

        if (b == current_band)
          be.freq(rig_ptr -> rig_frequency());
        else
...
// BATCH MESSAGES window
      ...
      { try
        { string current_message { };
          ...
          for (const auto& messages_line : to_lines <string> (read_file(context_path, context.batch_messages_file())))
          { if (!messages_line.empty())
            { if (messages_line.contains(LEFT_SQUARE_BRACKET))
                current_message = delimited_substring <string> (messages_line, LEFT_SQUARE_BRACKET, RIGHT_SQUARE_BRACKET, DELIMITERS::DROP);       // extract this batch message
              else
                batch_messages += { remove_peripheral_spaces <string> (messages_line) /* callsign */, current_message };       // associate this message with the callsign on the line
            }
          }
...
// BCALL window
      ...
      win_bcall < WINDOW_BOLD <= EMPTY_STR;
...
// CALL window
      ...
      win_call < WINDOW_BOLD <= EMPTY_STR;
...
// CALL HISTORY window
      ...
      win_call_history <= WINDOW_CLEAR;                                        // make it visible
...
// possibly set speed of internal keyer. Direct quote from N2IC: Contesters don't use internal keyers. [Message-ID: <515996A2.9060800@arrl.net>]
      ...
      { if (context.sync_keyer())
          rig_ptr -> keyer_speed(context.cw_speed());
      }

      catch (const rig_interface_error& e)
      { alert("Error setting CW speed on rig"sv);
      }
...
// start to display the rig status (in the RIG window); also get rig frequency for bandmap
      jthread(display_rig_status, 1000ms, rig_ptr).detach();
...
// octothorpe
        if (logbk.size() >= 1)
        { if (const QSO& last_qso { logbk[logbk.size()] }; rules.sent_exchange_includes("SERNO"sv, last_qso.mode()))   // logbook is wrt 1
            octothorpe = from_string<unsigned int>(last_qso.sent_exchange("SERNO"sv)) + 1;
        }
        else
          octothorpe = 1;
...
// go to band and mode of last QSO
          ...
          rig_ptr -> rig_frequency(frequency(last_qso.freq()));
          rig_ptr -> rig_mode(m);
          ...
          rig_ptr -> base_state();  // turn off RIT, split, sub-rx
        }
...
/*! \brief          Display band and mode
    ...
    \param  m       mode
*/
void display_band_mode(window& win, const BAND b, const MODE m)
...
// this is a good opportunity to check for exiting
      ...
      win_time < CURSOR_START_OF_LINE <= substring <string> (string(buf.data(), 26), 11, 8);  // extract HH:MM:SS and display it
...
// see whether the band has changed; max of five attempts
            for ( [[maybe_unused]] int n { 1 }; need_to_set_band and (n != 5); ++n)
            { sleep_for(500ms);

              new_f = rig_ptr -> rig_frequency();

// 250509
              new_sgb = to_BAND(new_f);
              need_to_set_band = (current_band != to_BAND(new_f));    // is there still a mismatch?
            ...
            { ost << "Consistent band mismatch during poll; new_sgb = " << new_sgb << ", new_f = " << new_f << ", BAND(new_f) = " << to_BAND(new_f) << "; setting band" << endl;
              ...
              const BAND b { to_BAND(new_f) };    // this is the actual frequency/band of VFO A
          ...
          const string rit_xit_str { build_rit_xit_str(status) };

          rig_is_split = status.split_enabled();

          const string bandwidth_str   { status.bw_str() };
          const string frequency_b_str { f_b.display_string() };
          const string centre_str      { status.centre_str() };
...
// now display the status
          ...
          const bool is_locked { status.is_locked() };

          if (status.xit_enabled() and is_locked and !locked_warning)
          { locked_warning = true;
          ...
          if (locked_warning)
          { if (!status.xit_enabled() or !is_locked)
            { locked_warning = false;

              win_message <= WINDOW_CLEAR;       // clear the MESSAGE window

// force RIT to be enabled if we are locked
              if (is_locked and !(status.rit_enabled()) )
                rigp -> rit_enable();
            }
          }

          const bool sub_rx { status.sub_rx_enabled() };
...
// don't change the bandwidth if the rig has returned a ridiculous value, which happens occasionally with the K3 (!!)
        ...
        }                                               // end block that is executed only of the status lookup was valid

// possibly check the RX ANT status
        if (win_rx_ant.defined())                     // don't do anything if the window isn't defined
        { const bool   rx_ant_in_use   { status.rx_ant() };
...
// I don't understand why the scrolling occurs automatically... in particular, I don't know what causes it to scroll
      ...
      { win_cluster_screen < lines[n];                       // THIS causes the scroll, but I don't know why
        ...
        if ( (n != (lines.size() - 1)) or no_cr.ends_with(LF) )
...
//        const string  msg           { "NO DATA RECEIVED FOR "s + to_string(N_SECONDS(time_since_data_last_received)) + " SECONDS; RESET IN "s + to_string(N_SECONDS(time_to_reset)) + " SECONDS"};
        ...
        cluster_line_win < WINDOW_CLEAR < COLOURS(COLOUR_RED, COLOUR_BLACK) < centre(msg, 0) <= COLOURS(fg_colour, bg_colour);
...
// eliminate some obvious busts
          ...
          if ( (first == SLASH) or (first == 'Q') or (first == '0') )    // 1A0C is a valid call
            continue;

          if (call.ends_with(SLASH))
            continue;
...
// record as the most recent QRG for this station
            const int band_nr { to_int(BAND(post.freq())) };
...
// highlight it if it's on our current band
                    if ( (dx_band == cur_band) or is_me)
                      cluster_mult_win < WINDOW_HIGHLIGHT;       // swaps fg/bg

                    if (is_me)
                      cluster_mult_win < WINDOW_BOLD;            // call in bold darkish blue

                    cluster_mult_win < pad_right(frequency_str + SPACE + dx_callsign, cluster_mult_win.width());  // display it -- removed refresh
...
// add the post to the correct bandmap unless it's a marked frequency
    ...
    { ignore_next_process_insertion_queue = false;
      ...
      sleep_for(1s);                // give it time to be to be set to true again, if necessary
    }

    BAND displayed_band { bandmap_display_band };   // band to be displayed
    ...
    { if (b == displayed_band)
      { while (ignore_next_process_insertion_queue)
        ...
        displayed_band = bandmap_display_band;

        if (b == displayed_band)
          bandmaps[to_uint(b)].process_insertion_queue(bandmap_insertion_queues[to_uint(b)], bandmap_win);
        else
        { ost << "Displayed band appears to have changed; not writing to window; displayed band now = " << displayed_band << endl;
          bandmaps[to_uint(b)].process_insertion_queue(bandmap_insertion_queues[to_uint(b)]);
        }
      }
      else
        bandmaps[to_uint(b)].process_insertion_queue(bandmap_insertion_queues[to_uint(b)]);
...
// [ and ] (for regex)
  if (!processed and e.is_unmodified() and (e.is_char(LEFT_SQUARE_BRACKET) or e.is_char(RIGHT_SQUARE_BRACKET)))
...
// . + -
  if (!processed and ( e.is_char(DOT) or (e.symbol() == XK_minus) or (e.is_unmodified() and ( (e.symbol() == XK_KP_Add)) ))) // XK_KP_Subtract is now CW bandwidth toggle
    processed = (win <= e.str(), true);

// need comma and asterisk for rescore options, backslash for scratchpad
  if (!processed and (e.is_char(COMMA) or e.is_char(ASTERISK) or e.is_char(BACKSLASH)))
    processed = (win <= e.str(), true);

// question mark, which is displayed in response to pressing the equals sign
  if (!processed and e.is_unmodified() and e.is_char(EQUALS))
    processed = (win <= QUESTION_MARK, true);

  const string call_contents { remove_peripheral_spaces <string> (win.read()) };
  const BAND   cur_band      { current_band };
...
// get the frequency for the post
    auto get_frequency = [] (void)
    {
      ...
      return rig_ptr -> rig_frequency().display_string();
    };
...
// In CQ mode, this is a self-spot
    ...
    { const string& callsign { my_call };
      ...
      if (spot_status)
        alert("posted self spot: "s + callsign + ", " + qrg + ", " + comment);
      else
        alert("error posting self spot: "s + callsign + ", " + qrg + ", " + comment);
    }
...
// if call window is not empty and we are in SAP mode, send the contents of the call window, which is expected to be "call qrg"
    ...
    { if (call_contents.contains(SPACE))         // if it contains a space, just send the contents; currently there is no way for the CALL window to contain a space
      { const bool spot_status { cluster_p -> spot(call_contents) };
        ...
        if (spot_status)
          alert("posted spot: "s + call_contents);
        else
          alert("error posting spot: "s + call_contents);
      }
      ...
      { const string& callsign { call_contents };
        ...
        if (spot_status)
          alert("posted spot: "s + callsign + ", " + qrg + ", " + comment);
        else
          alert("error posting spot: "s + callsign + ", " + qrg + ", " + comment);
      }
    }
...
//      time_log <std::chrono::milliseconds> tl;
      ...
      { const frequency set_last_f { rig_ptr -> rig_frequency() };
        ...
        rig_ptr -> set_last_frequency(cur_band, cur_mode, set_last_f);             // save current frequency

        { if ( BAND(rig_ptr -> get_last_frequency(cur_band, cur_mode)) != cur_band )
          { alert("ERROR: inconsistency in frequency/band info"s);
            ...
            ost << "  get_last_frequency = " << rig_ptr -> get_last_frequency(cur_band, cur_mode) << endl;
            ost << "  BAND(get_last_frequency) = " << BAND(rig_ptr -> get_last_frequency(cur_band, cur_mode)) << endl;
        ...
        const MODE     dest_mode { (permitted_modes.size() == 1) ? *(permitted_modes.begin()) : cur_mode };
        const bandmode bmode     { new_band, dest_mode };

        frequency last_frequency { rig_ptr -> get_last_frequency(bmode) };   // go to saved frequency for this band/mode (if any)
...
// check that we're about to go to the correct band
        ...
        rig_ptr -> rig_frequency(last_frequency);

// confirm that it's really happened
        { const auto f { rig_ptr -> rig_frequency() };

          ost << "new frequency we have moved to appears to be: " << f << endl;
//          ost << "new band is supposed to be: " << new_band << ", band name = " << BAND_NAME[static_cast<unsigned int>(new_band)] << "m" << endl;
//          ost << "new band is actually: " << BAND(f) << ", band name = " << BAND_NAME[static_cast<unsigned int>(BAND(f))] << "m" << endl;
          ost << "new band is supposed to be: " << new_band << ", band name = " << to_string(new_band) << "m" << endl;
          ost << "new band is actually: " << BAND(f) << ", band name = " << to_string(BAND(f)) << "m" << endl;

          ost << "the value of current_band is: " << static_cast<BAND>(current_band) << endl;
//          ost << "the value of the current mode is: " << MODE_NAME.at(cur_mode) << endl;
//          ost << "the value of the mode after band change is: " << MODE_NAME.at(rig_ptr -> rig_mode()) << endl;
          ost << "the value of the current mode is: " << ::to_string(cur_mode) << endl;
          ost << "the value of the mode after band change is: " << to_string(rig_ptr -> rig_mode()) << endl;
        }
...
// these are the commands that take a lot of time
        rig_ptr -> rig_mode(dest_mode);
        current_mode = dest_mode;

//        ost << "the value of the mode after explicitly setting it to " << MODE_NAME[dest_mode] << " is: " << MODE_NAME.at(rig_ptr -> rig_mode()) << endl;
        ost << "the value of the mode after explicitly setting it to " << to_string(dest_mode) << " is: " << to_string(rig_ptr -> rig_mode()) << endl;

        enter_sap_mode();
        rig_ptr -> base_state();    // turn off RIT, split and sub-rx

// clear the call window (since we're now on a new band)
        win < WINDOW_CLEAR <= CURSOR_START_OF_LINE;
        display_band_mode(win_band_mode, new_band, dest_mode);

// update bandmap
        FOR_ALL(bandmaps, [] (bandmap& bm) { bm.increment_version(); });  // to handle case of multiple quick band changes

        bandmap& bm { bandmaps[static_cast<unsigned int>(new_band)] };

//        ost << "displaying band map for band: " << BAND_NAME[static_cast<unsigned int>(new_band)] << 'm' << endl;
        ost << "displaying band map for band: " << to_string(new_band) << 'm' << endl;
...
// is there a station close to our frequency?
        const string nearby_callsign { bm.nearest_displayed_callsign(last_frequency, context.guard_band(dest_mode)) };

        display_nearby_callsign(nearby_callsign);  // clears NEARBY window if call is empty

// update displays of needed mults
        update_remaining_callsign_mults_window(statistics, string { }, new_band, dest_mode);
        update_remaining_country_mults_window(statistics, new_band, dest_mode);
        update_remaining_exchange_mults_windows(statistics, new_band, dest_mode);

// clear some windows
        win_last_qrg          < WINDOW_CLEAR <= CURSOR_START_OF_LINE;
        win_putative_exchange < WINDOW_CLEAR <= CURSOR_START_OF_LINE;
        win_scp               < WINDOW_CLEAR <= CURSOR_START_OF_LINE;
...
// ALT-M -- change mode
  ...
  { rig_ptr -> set_last_frequency(cur_band, cur_mode, rig_ptr -> rig_frequency());             // save current frequency
    ...
    rig_ptr -> rig_frequency(rig_ptr -> get_last_frequency(bmode).hz() ? rig_ptr -> get_last_frequency(bmode) : DEFAULT_FREQUENCIES.at(bmode));
    rig_ptr -> rig_mode(new_mode);
...
// F11 -- band map filtering
  ...
  { const string contents { remove_peripheral_spaces <string> (win.read()) };  // can't use string_view here
    ...
    bandmap& bm = bandmaps[static_cast<unsigned int>(cur_band)];        // use current bandmap to make it easier to display column offset
...
//        win_score_bands < WINDOW_CLEAR < "Score Bands: "s <= join( SR::to<vector>(rules.score_bands() | SRV::transform([] (const BAND b) { return BAND_NAME[static_cast<unsigned int>(b)]; })), SPACE );
        win_score_bands < WINDOW_CLEAR < "Score Bands: "s <= join(rules.score_bands(), SPACE);
...
//
//        win_score_modes < WINDOW_CLEAR < "Score Modes: "s <= join( SR::to<vector>(rules.score_modes() | SRV::transform([] (const MODE m) { return MODE_NAME[m]; })), SPACE );
        win_score_modes < WINDOW_CLEAR < "Score Modes: "s <= join(rules.score_modes(), SPACE);
...
// if there's a letter, it's not a frequency; otherwise we try to parse as a frequency
    ...
    { const bool contains_letter { contains_upper_case_letter(contents) };
      ...
      { const bool contains_plus  { (contents[0] == PLUS) };        // '+' can be entered from the keypad w/o using shift
        const bool contains_minus { (contents[0] == MINUS) };
...
// handle frequency without the MHz part: [n][n]n.n
          ...
          { const bool possible_qsy { ( (contents.length() >= 3) and (contents[contents.size() - 2] == DOT) ) };

            if (possible_qsy)
            { const float band_edge_in_khz { rig_ptr -> rig_frequency().lower_band_edge().khz() };

              switch (cur_band)
              { case BAND_160 :
                  value += (value < 100 ? 1'800 : 1'000);
                  break;

                case BAND_80 :
                  value += (value < 100 ? 3'500 : 3'000);
                  break;
          ...
          const frequency cur_rig_frequency { rig_ptr -> rig_frequency() };
          const frequency new_frequency     { ( (contains_plus or contains_minus) ? cur_rig_frequency.hz() + (value * 1'000) : value ) };
          ...
          if (valid)
          { ok_to_poll_rig = false;                                                 // (probably) stop polling while we change frequency
            
            const BAND cur_band { to_BAND(cur_rig_frequency) };                     // hide old cur_band

            rig_ptr -> set_last_frequency(cur_band, cur_mode, cur_rig_frequency);             // save current frequency
            rig_ptr -> rig_frequency(new_frequency);
...
// update displays of needed mults
              ...
              update_remaining_exchange_mults_windows(statistics, cur_band, m);
            }
...
// if we're in SAP mode, don't call him if he's a dupe
      ...
      { const cursor posn { win.cursor_position() };
        ...
        be.freq(rig_is_split ? rig_ptr -> rig_frequency_b() : rig_ptr -> rig_frequency());  // the TX frequency
        be.expiration_time(be.time() + context.bandmap_decay_time_local() * 60);
        be.calculate_mult_status(rules, statistics);

        bandmap& bandmap_this_band { bandmaps[static_cast<unsigned int>(cur_band)] };
...
// need to figure out a way to generalise all this
          ...
          { if (exf.name() == "ITUZONE+SOCIETY"sv)
            { string iaru_guess { exchange_db.guess_value(contents, "SOCIETY"sv) };      // start with guessing it's a society

              if (iaru_guess.empty())
                iaru_guess = to_upper(exchange_db.guess_value(contents, "ITUZONE"sv));   // try ITU zone if no society
          ...
          { exchange_str += ( (cur_mode == MODE_CW) ? "599 "sv : "59 "sv );
            processed_field = true;
          }
...
// add to bandmap if we're in SAP mode
      ...
      { bandmap_entry be;
        ...
        be.freq(rig_is_split ? rig_ptr -> rig_frequency_b() : rig_ptr -> rig_frequency());        // also sets band; TX frequency
        be.expiration_time(be.time() + context.bandmap_decay_time_local() * 60);
        be.calculate_mult_status(rules, statistics);

        bandmap& bandmap_this_band { bandmaps[static_cast<unsigned int>(be.band())] };
...
// CTRL-KP-ENTER -- look for, and then display, entry in all the bandmaps
  ...
  { string results;
    ...
    { bandmap& bm { bandmaps[static_cast<unsigned int>(b)] };            // use current bandmap to make it easier to display column offset
...
// SPACE -- generally, dupe check
  if (!processed and e.is_char(SPACE))
  {
// if we're inside a command, just insert a space in the window; also if we are writing a comment
    if ( ((original_contents.size() > 1) and (original_contents[0] == DOT)) or original_contents.contains(BACKSLASH))
      win <= SPACE;
...
// possibly put a bandmap call into the call window
      ...
      { if (const string nearby_contents { remove_peripheral_spaces <string> (win_nearby.read()) }; !nearby_contents.empty())
        { win < CURSOR_START_OF_LINE <= nearby_contents;
          display_call_info(nearby_contents);
        }
      }

      const string current_contents { remove_peripheral_spaces <string> (win.read()) };    // contents of CALL window may have changed, so we may need to re-insert/refresh call in bandmap
...
// and unmodified ; and ': also up or down to next stn that matches the N7DR criteria
  if (!processed and e.is_unmodified() and ( e.is_char(SEMICOLON) or e.is_char(APOSTROPHE) ) )
  { if (drlog_mode == DRLOG_MODE::SAP)                              // do nothing in CQ mode
    { ignore_next_poll = true;                                      // briefly inhibit window updates from rig polling; possibly move this into process_bandmap_function()
      ignore_next_process_insertion_queue = true;
      update_quick_qsy();
      processed = process_bandmap_function(e.is_char(SEMICOLON) ? DOWN : UP);
    }
    else
      processed = true;
  }
...
// CTRL-; and CTRL-' -- up or down to stn that matches the N7DR criteria, in increments of 5 stns
  if (!processed and (e.is_control(SEMICOLON) or e.is_control(APOSTROPHE)))
  { if (drlog_mode == DRLOG_MODE::SAP)                              // do nothing in CQ mode
    { ignore_next_poll = true;                                      // briefly inhibit window updates from rig polling; possibly move this into process_bandmap_function()
      ignore_next_process_insertion_queue = true;
      update_quick_qsy();
      processed = process_bandmap_function(e.is_control(SEMICOLON) ? DOWN : UP, 4);  // move by 5 stations
    }
    else
      processed = true;
  }
...
// ALT-; and ALT-' -- up or down to stn that matches the N7DR criteria, in increments of a large number of stns
  if (!processed and (e.is_alt(SEMICOLON) or e.is_alt(APOSTROPHE)))
  { if (drlog_mode == DRLOG_MODE::SAP)                              // do nothing in CQ mode
    { ignore_next_poll = true;                                      // briefly inhibit window updates from rig polling; possibly move this into process_bandmap_function()
      ignore_next_process_insertion_queue = true;
      update_quick_qsy();

      const int16_t nskip { static_cast<int16_t>( (bandmaps[static_cast<unsigned int>(cur_band)].cull_function() == 1) ? (win_bandmap.height() - 1) : 24 ) };

      processed = process_bandmap_function(e.is_alt(SEMICOLON) ? BANDMAP_DIRECTION::DOWN : BANDMAP_DIRECTION::UP, nskip);  // move by (nskip + 1) stations
    }
    else
      processed = true;
  }
...
// remove the last line from the log on disk
        const vector<string> disk_log_lines { to_lines <string> (read_file(context.logfile())) };
...
// remove any duplicates from all_matches while maintaining ordering
        ...
        { if (scp_index == -1)
          { scp_index = 0;                                // go to first call
            ...
            if (all_matches[scp_index] == remove_peripheral_spaces <string> (win.read()))               // there was a best match and it's the same as the first SCP/fuzzy match
...
// ` -- SWAP RIT and XIT
  if (!processed and e.is_char(BACKTICK))
    processed = swap_rit_xit();

// ALT-P -- Dump bandscope
  if (!processed and e.is_alt('p') and context.bandscope())
    processed = (rig_ptr -> bandscope_screenshot(context.bandscope_snapshot_file()), true);
...
// CTRL-S -- toggle split
  if (!processed and e.is_control('s'))
  { try
    { rig_ptr -> split_enabled() ? rig_ptr -> split_disable() : rig_ptr -> split_enable();
    }
...
// try to parse as frequency
    ...
    const frequency f_b             { rig_ptr -> rig_frequency_b() };

    if (!contains_letter)    // try to parse as frequency
    { const bool contains_plus  { (original_contents[0] == PLUS) };        // this can be entered from the keypad w/o using shift
      const bool contains_minus { (original_contents[0] == MINUS) };
...
// if there's a plus or minus we interpret the value as kHz and move up or down from the current QRG
      {
// handle frequency without the MHz part: [n][n]n.n
        if (!contains_plus and !contains_minus and (value < 1'000))
        { bool possible_qsy { (original_contents.length() >= 3) };

          possible_qsy = possible_qsy and (original_contents[original_contents.size() - 2] == DOT);
...
// get band of VFO B
        ...
        const frequency new_frequency_b { ( (contains_plus or contains_minus) ? f_b.hz() + (value * 1'000) : value ) };
...
// assume it's a call -- look for the same call in the VFO B bandmap
      bandmap_entry be { bandmaps[static_cast<unsigned int>(band_b)][original_contents] };

      if (!(be.callsign().empty()))
        rig_ptr -> rig_frequency_b(be.freq());
      else                                          // didn't find an exact match; try a substring search
      { be = bandmaps[static_cast<unsigned int>(band_b)].substr(original_contents);
...
// ALT--> -- VFO A -> VFO B
  if (!processed and (e.is_alt(GREATER_THAN) or e.is_alt(DOT)))
    processed = ( rig_ptr -> rig_frequency_b(rig_ptr -> rig_frequency()), true );

// ALT-<- -- VFO B -> VFO A
  if (!processed and (e.is_alt(LESS_THAN) or e.is_alt(COMMA)))
    processed = ( rig_ptr -> rig_frequency(rig_ptr -> rig_frequency_b()), true );
...
// CTRL-N -- Place NEARBY call into CALL window and update QSL window
  ...
  { if (win_call.empty() and !win_nearby.empty())
    { const string new_call { remove_peripheral_spaces <string> (win_nearby.read()) };
...
// ALT-R -- toggle RX antenna
  ...
  { rig_ptr -> toggle_rx_ant();
    processed = update_rx_ant_window();
  }
  
// CTRL-= -- quick QSY (should this be based on displayed band instead of current band?)
  if (!processed and (e.is_control(EQUALS)))
  { auto [old_frequency, old_mode] { quick_qsy_map.at(current_band) };

    const auto [ f, m ] { get_frequency_and_mode() };

    rig_ptr -> rig_frequency(old_frequency);
    rig_ptr -> rig_mode(old_mode);
...
//                  <= pad_left(f.display_string(), 7) + SPACE + MODE_NAME[m];
                  <= pad_left(f.display_string(), 7) + SPACE + to_string(m);
...
// KP5 -- centre RIT if on SSB and RIT is on
  ...
  { if ( (current_mode == MODE_SSB) and rig_ptr -> rit_enabled() )
      rig_ptr -> rit(0);
...
// ALT-G go to the frequency in win_last_qrg
  ...
  { const string contents { win_last_qrg.read() };
    ...
    if (const size_t posn { contents.find(COLON) }; (posn != string::npos) and (posn != (contents.size() - 1)))
    { const frequency f { remove_peripheral_spaces <string> (substring <string> (contents, posn + 1)) };

      rig_ptr -> rig_frequency(f);
...
// finished processing a keypress
  ...
  { if (win_call.empty())
    ...
    { if (const string current_contents { remove_char(remove_peripheral_spaces <string> (win.read()), BACKSLASH) }; current_contents != original_contents)   // remove any \ characters
...
// APOSTROPHE
  if (!processed and e.is_char(APOSTROPHE))
    processed = (win <= e.str(), true);
...
// CTRL-S -- send contents of CALL window to scratchpad
  if (!processed and e.is_control('s'))
    processed = send_to_scratchpad(remove_peripheral_spaces <string> (win_call.read()));
...
// COMMA -- place contents of call window into this window, preceded by a dot
  if (!processed and e.is_char(COMMA))
//    processed = (win <= (DOT_STR + remove_peripheral_spaces <string> (win_call.read())), true);
    processed = (win <= (DOT + remove_peripheral_spaces <string> (win_call.read())), true);

// FULL STOP
  if (!processed and e.is_char(DOT))
    processed = (win <= DOT, true);
...
// ENTER, KP_ENTER, ALT-Q -- thanks and log the contact; also perhaps start QTC process
  ...
  { const BAND   cur_band      { current_band };
    ...
    string         exchange_contents     { squash(remove_peripheral_spaces <string> (win_exchange.read())) };
    vector<string> exchange_field_values { split_string <string> (exchange_contents, SPACE) };

    string new_rst;

// figure out whether we have sent a different RST (in SKCC)
    constexpr char rst_character { APOSTROPHE };    // apostrophe

    if (exchange_contents.contains(rst_character))
    { const size_t last_apostrophe { exchange_contents.find_last_of(rst_character) };
      const size_t next_space      { exchange_contents.find_first_of(SPACE, last_apostrophe + 1) };

      size_t word_length;

      word_length = ( ( (next_space == string::npos) ? exchange_contents.length() : (next_space - 1) ) - last_apostrophe );
      new_rst     = substring <string> (exchange_contents, last_apostrophe + 1, word_length);
      ...
      { const parsed_exchange pexch { from_callsign, canonical_prefix, rules, cur_mode, exchange_field_values };  // this is relatively slow, but we can't send anything until we know that we have a valid exchange
        ...
        { if ( (cur_mode == MODE_CW) and (cw_p) )         // don't acknowledge yet if we're about to send a QTC
          ...
          { if ( (cur_mode == MODE_CW) and (cw_p) and (drlog_mode == DRLOG_MODE::SAP))    // in SAP mode, he doesn't care that we might have changed his call
            ...
            if ( (cur_mode == MODE_CW) and (cw_p) and (drlog_mode == DRLOG_MODE::CQ))    // in CQ mode, he does so care
...
// callsign is the last entry in the CALL window
          ...
          { const vector<string> call_contents_fields { split_string <string> (call_contents, SPACE) };
            ...
            qso.freq( frequency( rig_is_split ? rig_ptr -> rig_frequency_b() : rig_ptr -> rig_frequency() ).display_string() );    // in kHz; 1dp

// build name/value pairs for the sent exchange
            vector<pair<string, string> > sent_exchange { context.sent_exchange(qso.mode()) };

            for (auto& [ _, value ] : sent_exchange)
            { if (value == "#"sv)
                value = serial_number_string(octothorpe);
            }
...
// in SKCC, we aren't using the computer to send CW, so we can determine the sent RST now, long after the fact
            ...
            if (!new_rst.empty())
            { for (auto& [ name, value ] : sent_exchange)
              { if (name == "RST"sv)
                  value = new_rst;
              }
            }
...
// if we are in CQ mode, then remove this call if it's present elsewhere in the bandmap ... we assume he isn't CQing and SAPing simultaneously on the same band
          bandmap& bandmap_this_band { bandmaps[to_uint(cur_band)] };
...
// add the stn to the bandmap
             ...
             be.freq( rig_is_split ? rig_ptr -> rig_frequency_b() : rig_ptr -> rig_frequency() );
...
// CTRL-T -- delete word
  ...
  { const cursor         original_posn { win.cursor_position() };
    ...
    { const bool is_space { (contents[original_posn.x()] == SPACE) };
...
// put cursor in correct window
      if (remove_peripheral_spaces <string> (win_exchange.read()).empty())        // go to the CALL window
      { win_call.move_cursor(call_contents.find(SPACE), 0);            // first empty space
        win_call.refresh();
        set_active_window(ACTIVE_WINDOW::CALL);
...
// . or space
  if ( e.is_char(DOT) or e.is_char(SPACE) )    // to permit editing of frequency
  { win <= e.str();
    goto FINISHED_PROCESSING_LOG_WINDOW;    // could just return
  }
...
// get the original QSOs that were in the window
        ...
        unsigned int qso_number  { static_cast<unsigned int>(logbk.size()) };   // size() returns a long unsigned int
...
// do this just for QSOs that have changed
            ...
            { bandmap& bm { bandmaps[static_cast<unsigned int>(qso.band())] };
...
void enter_cq_mode(void)
{
  ...
  { if (context.cq_auto_lock())
    ...
    rig_ptr -> bandscope_span(bandscope_span_cq);
...
{ SAFELOCK_SET(drlog_mode_mutex, drlog_mode, DRLOG_MODE::SAP);
  ...
  { rig_ptr -> unlock();
    ...
    rig_ptr -> disable_xit();
    rig_ptr -> disable_rit();
    rig_ptr -> bandscope_span(bandscope_span_sap);
  }
...
{ if (win_call_history.valid())
  ...
  { //const string qtc_str { LEFT_SQUARE_BRACKET + to_string(qtc_db.n_qtcs_sent_to(callsign)) + RIGHT_SQUARE_BRACKET };
    const string qtc_str { delimit(to_string(qtc_db.n_qtcs_sent_to(callsign)), SQUARE_BRACKETS) };
...
//    const string name_plus_continent_str { name_str + " ["s + location_db.continent(callsign) + RIGHT_SQUARE_BRACKET };
    const string name_plus_continent_str { name_str + SPACE + delimit(location_db.continent(callsign), SQUARE_BRACKETS) };
    ...
    { if (n_modes > 1)
        win_info < cursor(0, next_y_value--) < WINDOW_REVERSE < centred_string(to_string(this_mode), win_info.width()) < WINDOW_NORMAL;

// QSOs
      string line { pad_right("QSO"sv, FIRST_FIELD_WIDTH) };

      for (const BAND b : permitted_bands)
        line += pad_left( ( q_history.worked(callsign, b, this_mode) ? "-"s : to_string(b) ), FIELD_WIDTH);
...
//          line = pad_right("Country ["s + canonical_prefix + RIGHT_SQUARE_BRACKET, FIRST_FIELD_WIDTH);
          line = pad_right("Country "s + delimit(canonical_prefix, SQUARE_BRACKETS), FIRST_FIELD_WIDTH);

          for (const BAND b : permitted_bands)
          { const string per_band_indicator { known_country_mults.contains(canonical_prefix) ? (statistics.is_needed_country_mult(callsign, b, this_mode, rules) ? to_string(b) : "-"s )
                                                                                             : to_string(b)
                                            };
...
// exch mults
      ...
      { if (const bool output_this_mult { rules.is_exchange_field_used_for_country(exch_mult_field, canonical_prefix) }; output_this_mult)
        { const string exch_mult_value { exchange_db.guess_value(callsign, exch_mult_field) };      // make best guess as to the value of this field
          ...
          line = pad_right(exch_mult_field + SPACE + delimit(exch_mult_value, SQUARE_BRACKETS), FIRST_FIELD_WIDTH);

          for (const BAND b : permitted_bands)
            line += pad_left( ( statistics.is_needed_exchange_mult(exch_mult_field, exch_mult_value, b, this_mode) ? to_string(b) : "-"s ), FIELD_WIDTH);
...
// PUTATIVE EXCHANGE window
      ...
      { if (const string expected_exchange { expected_received_exchange(callsign) }; !expected_exchange.empty())
        { const string msg { centred_string(delimit(expected_exchange, SQUARE_BRACKETS), win_putative_exchange.width()) };
...
// callsign mults
      ...
      { string callsign_mult_value { };
        ...
        { line = pad_right(callsign_mult + SPACE + delimit(callsign_mult_value, SQUARE_BRACKETS), FIRST_FIELD_WIDTH);

          for (const BAND b : bands)
            line += pad_left( ( statistics.is_needed_callsign_mult(callsign_mult, callsign_mult_value, b, this_mode) ? to_string(b) : "-"s ), FIELD_WIDTH);
...
{ const bool contains_octothorpe { msg.contains(OCTOTHORPE) };
  const bool contains_per_cent   { msg.contains(PER_CENT) };
  const char t_char_to_send      { t_char(long_t) };   // the character to send for a zero

  string serno_replaced;

  if (contains_octothorpe or contains_per_cent)
  { string serno_str { to_string(octothorpe) };      // convert serial number to string

    if (!context.short_serno())                           // if send leading Ts
      serno_str = pad_left(serno_str, (octothorpe < 1000 ? 3 : 4), 'T');  // always send at least three characters in a serno, because predictability in exchanges is important

    if (serno_spaces)                                               // number of short spaces to add when sending serno
    { const string spaces { create_string(CARET, serno_spaces) };   // string of spaces to insert between each character
      const string tmp    { serno_str };

      serno_str.clear();

      for_each(tmp.cbegin(), prev(tmp.cend()), [spaces, &serno_str] (const char c) { serno_str += (c + spaces); } );  // add spaces after all except last character

      serno_str += last_char(tmp);
    }
    ...
    { const int  n_to_find    { (octothorpe < 10 ? 2 : 1) };
      ...
      for (size_t n { 0 }; !found_all and (n < serno_str.size() - 1); ++n)
      { if ( !found_all and (serno_str[n] == 'T') )
        { serno_str[n] = t_char_to_send;
          found_all = (++n_found == n_to_find);
        }
      }
    }

    string percent_str { serno_str };

    if (contains_per_cent)
    { for (size_t idx { 0 }; idx < percent_str.size(); ++idx)
      { if (percent_str[idx] == '0')
          percent_str[idx] = t_char_to_send;

        if (percent_str[idx] == '9')
          percent_str[idx] = 'N';
      }
    }

    serno_replaced = replace(msg, OCTOTHORPE, serno_str);
    serno_replaced = replace(serno_replaced, PER_CENT, percent_str);
  }

//  const string at_replaced { replace( (octothorpe_replaced.empty() ? msg : octothorpe_replaced), "@"s, at_call) };
//  const string at_replaced { replace( (percent_replaced.empty() ? msg : percent_replaced), "@"s, at_call) };    // this doesn't look quite right
  const string at_replaced { replace( (serno_replaced.empty() ? msg : serno_replaced), COMAT, at_call) };
...
{ start_of_thread("simulator thread"s);
  ...
  const vector<string> qso_lines { to_lines <string> (file) };
...
// local function to perform the update
  auto perform_update = [force_known] (const string_view callsign_mult_name, const string& prefix)    // must be string, not string_view because of use of map<accumulator>
...
{ if (file_exists(archive_filename))
  { try
    { ifstream                        ifs { string { archive_filename } };            // the source archive; amazingly, there is still no string_view constructor for ifstream
      //ifstream ifs { archive_filename };
...
// statistics
      ...
      rig_ptr -> rig_frequency(rig_frequency);
    }
...
/*! \brief              Alert the user
    ...
    \param  show_time   whether to prepend HHMMSS and provide millisecond-reslution runtime
...
void alert(const string_view msg, const SHOW_TIME show_time)
{
  ...
  if (show_time == SHOW_TIME::SHOW)
    { win_message < now < SPACE; }

  win_message <= msg;
  ost << "ALERT: " << now << SPACE << css(static_cast<int>(global_timer.click())) << "ms: " << msg << endl;
}
...
{ if (win_local_time.wp())                                         // don't do it if we haven't defined this window
  { struct tm       structured_local_time;
    ...
    win_local_time < CURSOR_START_OF_LINE <= substring <string_view> (string { buf_local_time.data(), 26 }, 11, 5);  // extract HH:MM and display it
  }
}
...
{ ost << "Inside exit_drlog()" << endl;
  ...
  ost << "Number of type 1 posts processed: " << type_1_post_counter << endl;
  ost << "Number of type 2 posts processed: " << type_2_post_counter << endl;
...
{ string new_callsign { };
  ...
  { int n_green { 0 };            // count the number of green entries
    ...
    if ( (n_green == 1) and (do_not_return != tmp_callsign) )
...
// should never get here
  ost << "ERROR in is_needed_qso for " << callsign << ", " << to_string(b) << ", " << to_string(m) << endl;
...
{ const int shift_delta { (current_mode == MODE_CW)  ? shift_delta_cw : shift_delta_ssb };    // get the right shift for the mode
  ...
  { if (rig_ptr -> rit_enabled())
    { int last_rit { rig_ptr -> rit() };
      ...
      do
      { rig_ptr -> rit(last_rit + change);                 // this takes forever through hamlib
        last_rit += change;
...
{ win_cluster_line <= "UNCONNECTED"s;
  ...
  win_cluster_line < CURSOR_START_OF_LINE < WINDOW_CLEAR <= "CONNECTED"s;
...
{ win_rbn_line <= "UNCONNECTED"s;
  ...
  win_rbn_line < CURSOR_START_OF_LINE < WINDOW_CLEAR <= "CONNECTED"s;
...
{ if (callsign_mults_used)
  { string mult_name;
    ...
    if ( rules.callsign_mults().contains("AAPX"s) and (location_db.continent(callsign) == "AS"sv) )  // All Asian
    { qso.prefix(wpx_prefix(callsign));
      mult_name = "AAPX"s;
    }

    if ( rules.callsign_mults().contains("OCPX"s) and (location_db.continent(callsign) == "OC"sv) )  // Oceania
    { qso.prefix(wpx_prefix(callsign));
      mult_name = "OCPX"s;
    }

    if ( rules.callsign_mults().contains("SACPX"s) )      // SAC
    { qso.prefix(sac_prefix(callsign));
      mult_name = "SACPX"s;
    }

    if ( rules.callsign_mults().contains("UBAPX"s) and (location_db.canonical_prefix(callsign) == "ON"sv) )  // UBA
    { qso.prefix(wpx_prefix(callsign));
      mult_name = "UBAPX"s;
    }
...
// destination for the QTC is the callsign in the CALL window; or, if the window is empty, the call of the last logged EU QSO
    string destination_callsign { remove_peripheral_spaces <string> (win_call.read()) };
...
// check that we still have entries (this should always pass)
    ...
    { sending_qtc_series = true;
      ...
      win_qtc_status < WINDOW_CLEAR < CURSOR_START_OF_LINE < "Sending QTC "s < qtc_id < " to "s <= destination_callsign;
...
{ if (current_mode == MODE_CW)
  { const DRLOG_MODE current_drlog_mode { SAFELOCK_GET(drlog_mode_mutex, drlog_mode) };
    ...
    rig_ptr -> bandwidth( (current_drlog_mode == DRLOG_MODE::CQ) ? context.fast_cq_bandwidth() : context.fast_sap_bandwidth() );
  }
...
/*! \brief                    Process an F1 keystroke in the CALL window
 ...
 if CALL is empty, then: go to CQ mode on original frequency
                         turn off sub-rx
                         turn off split
                         clear bcall
...
// put cursor in correct window
    if (remove_peripheral_spaces <string> (win_exchange.read()).empty())        // go to CALL window
    { win_call.move_cursor(call_contents.find(SPACE), 0);               // first empty space
      win_call.refresh();
      set_active_window(ACTIVE_WINDOW::CALL);
      win_exchange.move_cursor(0, 0);
...
{ win_qtc_queue < WINDOW_CLEAR;
  ...
  { const unsigned int      win_height          { static_cast<unsigned int>(win_qtc_queue.height()) };
    ...
    for (const auto& qe : qtc_entries_to_send)
      win_qtc_queue <  reformat_for_wprintw(pad_left(to_string(index++), 2) + SPACE + qe.to_string(), win_qtc_queue.width());
  }
...
{ win_memories < WINDOW_CLEAR;
  ...
  { const cursor c_posn { 0, line_nr-- };
    ...
    win_memories < c_posn < to_string(number++) < SPACE < (me.freq()).display_string()
                          < pad_left(to_string(me.mode()), 5)
...
//  win_bandmap_filter < LEFT_SQUARE_BRACKET < to_string(bm.column_offset()) < "] "s <= bm.filter();
  win_bandmap_filter < delimit(to_string(bm.column_offset()), SQUARE_BRACKETS) < SPACE <= bm.filter();
}
...
{ if (win_bandmap_size.valid())
  { win_bandmap_size < WINDOW_CLEAR < centre("BM SIZE"s, win_bandmap_size.height() - 1);
...
// modelled after populate_win_call_history()
    ...
    { const cursor c_posn { 0, line_nr++ };
      ...
      win_bandmap_size < c_posn < pad_left(to_string(b), 3)                           // low band is on bottom
                       < pad_left(bandmaps[to_uint(b)].count_displayed_entries_no_markers(), 5);
    }
...
{ if (const string grid_name { exchange_db.guess_value(callsign, "GRID"sv) }; is_valid_grid_designation(grid_name))
...
{
// remove call from bandmap, and add it to the do_not_add list for that bandmap
  auto dns { [callsign] (bandmap& bm) { bm -= callsign;
                                        bm.do_not_add(callsign);
                                      } };

  if (b == ALL_BANDS)
    FOR_ALL(bandmaps, [&dns] (bandmap& bm) { dns(bm); });
  else                          // single band
//    dns(bandmaps[static_cast<unsigned int>(b)]);
    dns(bandmaps[to_uint(b)]);
...
// too much trouble
pair<adif3_record, int> first_qso_after(const vector<adif3_record>& vqsos, const int target_idate)
{ const auto it { FIND_IF(vqsos, [target_idate] (const adif3_record& rec) { return (rec.idate() >= target_idate); } ) };

  if (it == vqsos.end())
    return { adif3_record { }, -1 };

  const size_t idx { static_cast<size_t>(distance(vqsos.begin(), it)) };

  return { vqsos[idx], idx };
};
#endif
...
//    alert("read "s + comma_separated_string(old_adif3_log.size()) + " ADIF records from file: "s + context.old_adif_log_name(), SHOW_TIME::NO_SHOW);
    alert("read "s + css(old_adif3_log.size()) + " ADIF records from file: "s + context.old_adif_log_name(), SHOW_TIME::NO_SHOW);
...
{ if (current_mode == MODE_CW)
    rig_ptr -> bandwidth( (rig_ptr -> bandwidth() == cw_bandwidth_wide) ? cw_bandwidth_narrow : cw_bandwidth_wide );
...
{ constexpr int BANDWIDTH_PRECISION { 50 };        // K3 can set only to 50 Hz boundaries
  ...
  { enum SSB_AUDIO { SSB_WIDE,
    ...
    const SSB_AUDIO bw { (abs(rig_ptr -> bandwidth() - ssb_bandwidth_wide) < BANDWIDTH_PRECISION) ? SSB_NARROW : SSB_WIDE };

    ost << "bw = " << bw << endl;

    rig_ptr -> bandwidth( (bw == SSB_NARROW) ? ssb_bandwidth_narrow : ssb_bandwidth_wide );
    rig_ptr -> centre_frequency( (bw == SSB_NARROW) ? ssb_centre_narrow : ssb_centre_wide );
  }
...
// this should be roughly the same time as the timestamp we've put in the dx_post
    new_contents += substring <string> (hhmmss(), 0, 5) + SPACE + post.frequency_str() + "  "s + post.poster();
  }

  if (static_cast<int>(new_contents.size()) < win_height)
    new_contents += win_posted_by.snapshot();

  win_posted_by < WINDOW_CLEAR;
...
{ const string filename_suffix { (b == ALL_BANDS) ? EMPTY_STR : (DASH + to_string(b)) };
...
{ if (callsigns.empty())
  ...
  const string   filename_suffix { (b == ALL_BANDS) ? string { } : (DASH + to_string(b)) };
  const string   filename        { context.do_not_show_filename() + filename_suffix };

  ofstream outfile(filename);

  FOR_ALL(SR::to<CALL_SET>(callsigns), [&outfile] (const auto& callsign) { outfile << callsign << endl; });
}
...
// need to figure out a way to generalise all this
    if (exf.is_choice())
    { if (exf.name() == "ITUZONE+SOCIETY"sv)
      { string iaru_guess { exchange_db.guess_value(callsign, "SOCIETY"sv) };      // start with guessing it's a society

        if (iaru_guess.empty())
          iaru_guess = exchange_db.guess_value(callsign, "ITUZONE"sv);   // try ITU zone if no society
    ...
    if (exf.name() == "DOK"sv)
      return exchange_db.guess_value(callsign, "DOK"sv);

    if (!no_default_rst and (exf.name() == "RST"sv) and exf.is_mandatory())
      continue;
...
/// zoom bandscope to CQ mode, turn off RIT, turn on XIT
...
/*! \brief          Build the rit_xit_str string to be displayed
    \param  status  polled status
