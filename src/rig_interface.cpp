...
//          ost << "frequency mismatch: commanded = " << f << "; actual = " << rf << "; retrying" << endl;        // DEBUG for when we get stuck
  ...
  else    // frequency is not in a ham band
...
//      _force_hamlib = true;
  ...
  ost << "Rig structure initialised (via hamlib) for rig type "s + (rig_type.empty() ? "DUMMY"s : rig_type) << endl;
...
{ if (!_rig_connected)
  ...
  if (rig_set_split_vfo(_rigp, RIG_VFO_CURR, RIG_SPLIT_ON, RIG_VFO_B) != RIG_OK)  // magic parameters
...
//  const int status { rig_set_split_vfo(_rigp, RIG_VFO_A, RIG_SPLIT_OFF, RIG_VFO_A) };     // the line above also works
  ...
  if (rig_set_split_vfo(_rigp, RIG_VFO_A, RIG_SPLIT_OFF, RIG_VFO_A) != RIG_OK)
    _error_alert("Error executing command to disable SPLIT operation"s);
  else
    rig_is_split = false;
}
...
{ if (!_rig_connected)
  ...
  if (rig_get_split_vfo(_rigp, RIG_VFO_B, &split_mode, &tx_vfo) != RIG_OK)
  { _error_alert("Error getting SPLIT status"s);
    return false;
  }
...
{ if (!_rig_connected)                          // do nothing if no rig is connected
  ...
  if (rig_set_rit(_rigp, RIG_VFO_CURR, hz) != RIG_OK)
...
{ SAFELOCK(_rig);
  ...
  if (rig_get_rit(_rigp, RIG_VFO_CURR, &hz) != RIG_OK)
...
{ SAFELOCK(_rig);
  ...
  if (rig_set_xit(_rigp, RIG_VFO_CURR, hz) != RIG_OK)
    _error_alert("Hamlib error in xit(int)"s);         // handle this error upstairs
}
...
{ SAFELOCK(_rig);
  ...
  if (rig_set_func(_rigp, RIG_VFO_CURR, RIG_FUNC_LOCK, v) != RIG_OK)
...
{ if (SUB_RX())
 //   ost << "Currently unsupported function: attempt to control sub-receiver" << endl;
  { if (torf)
      ...
      _error_alert("Unsupported function: sub-receiver off");
  }
}
...
{ if (SUB_RX())
    _error_alert("Unsupported function: sub-receiver status");
...
{ SAFELOCK(_rig);
  ...
  if (rig_set_level(_rigp, RIG_VFO_CURR, RIG_LEVEL_KEYSPD, v) != RIG_OK)
...
{ SAFELOCK(_rig);
  ...
  if (rig_get_level(_rigp, RIG_VFO_CURR, RIG_LEVEL_KEYSPD, &v) != RIG_OK)
...
{ SAFELOCK(_rig);
  ...
  if (rig_get_func(_rigp, RIG_VFO_CURR, RIG_FUNC_LOCK, &v) != RIG_OK)
...
{ if (AUTO_NOTCH())
    _error_alert("Unimplemented function: rig_interface::toggle_notch_status()"sv);
  else
...
/*! \brief      Is split enabled?
    ...
    This interrogates the rig; it neither reads not writes the variable rig_is_split
*/
bool elecraft_k3_interface::split_enabled(void) const
{ if (!_rig_connected)
    return false;
...
/*! \brief      Set rit offset (in Hz)
    \param  hz  offset in Hz
*/
void elecraft_k3_interface::rit(const int hz) const
{ if (!_rig_connected)                          // do nothing if no rig is connected
    return;
...
/// is rit enabled?
bool elecraft_k3_interface::rit_enabled(void) const
...
/*! \brief      Set xit offset (in Hz)
    ...
    On the K3 this also sets the RIT offset
*/
void elecraft_k3_interface::xit(const int hz) const
{ if (hz == 0)                                // just clear the RIT/XIT
    raw_command("RC;"sv);
  else
  { const int    positive_hz { abs(hz) };
...
/// is sub-receiver on?
bool elecraft_k3_interface::sub_receiver(void) const
...
/*! \brief      Is the rig transmitting?
    ...
    or at least a paper.)
*/
bool elecraft_k3_interface::is_transmitting(void) const
{ constexpr int MAX_UNREACHABLE { 5 };              // maximum number of attempts before assuming rig is powered down; currently unused
...
/*! \brief      Set the bandwidth of VFO A
    \param  hz  desired bandwidth, in Hz
*/
void elecraft_k3_interface::bandwidth_a(const unsigned int hz) const
...
{ const string status_str { raw_command("IF;"sv, RESPONSE::EXPECTED) };               // K3 should return 38 characters
  ...
  { if (const k3_status rig_status { status_str }; rig_status)    // execute all this only if the response from the rig is sensible
    { const frequency f_a   { rig_status.freq() };                  // frequency of VFO A
      ...
      const MODE      m     { rig_interface::rig_mode() };          // mode
...
// hamlib seems to have no concept of a receive antenna

// hamlib seems to have no concept of a TEST mode that precludes transmission of RF
