...
// current seconds past midnight
  ...
  const uint64_t hh      { from_string<uint64_t>(substring <string_view> (now_str, 0, 2)) };
  const uint64_t mm      { from_string<uint64_t>(substring <string_view> (now_str, 3, 2)) };
  const uint64_t ss      { from_string<uint64_t>(substring <string_view> (now_str, 6, 2)) };
