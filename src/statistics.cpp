// $Id: statistics.cpp 299 2026-07-26 20:18:51Z  $
...
void running_statistics::_insert_callsign_mult(const string_view mult_name, const string_view mult_value, const BAND b, const MODE m)
...
//      mult.add_worked(mult_value, static_cast<BAND>(band_nr), static_cast<MODE>(mode_nr));      // add value and band for this mult name
      mult.add_worked(mult_value, b, m);      // add value and band for this mult name
    }
...
//      mult.add_worked(mult_value, static_cast<BAND>(band_nr), static_cast<MODE>(mode_nr));      // we've worked it
      mult.add_worked(mult_value, b, m);      // we've worked it
...
// QSOs
    ...
    { unsigned int qsos { 0 };
      ...
      { //const auto& nq_b { _n_qsos[m][static_cast<unsigned int>(b)] };
//        const auto& nq_b { _n_qsos[static_cast<unsigned int>(m)][static_cast<unsigned int>(b)] };
        const auto& nq_b { _n_qsos[to_uint(m)][to_uint(b)] };
...
//    unsigned int dupes_all_bands { 0 };
    ...
    { NTYPE dupes { 0 };
      ...
      for (const auto m : modes)
      { const NTYPE nd_mb { _n_dupes[to_uint(m)][to_uint(b)] };
...
// QSO points
    ...
    { unsigned int points_all_bands { 0 };
      ...
      { unsigned int points { 0 };
        ...
        for (const auto m : modes)
        { const auto& qp_mb { _qso_points[to_uint(m)][to_uint(b)] };
    ...
    { line = LF + pad_right("QTC QSOs"s, FIRST_FIELD_WIDTH);
      line += pad_left((to_string(_qtc_qsos_sent) + PIPE + to_string(_qtc_qsos_unsent)), FIELD_WIDTH * (permitted_bands.size() + 1));
...
*/
bool running_statistics::known_callsign_mult_name(const string_view putative_callsign_mult_name) const
{ SAFELOCK(statistics);
...
*/
bool running_statistics::is_needed_callsign_mult(const string_view mult_name, const string_view mult_value, const BAND b, const MODE m) const
{ SAFELOCK(statistics);
...
*/
bool running_statistics::is_needed_country_mult(const string_view callsign, const BAND b, const MODE m, const contest_rules& rules)
{ try
  { SAFELOCK(statistics);
...
{ const string mv { MULT_VALUE(exchange_field_name, exchange_field_value) };  // the mult value of the received field
  ...
  for (auto& [ field_name, mult ] : _exchange_multipliers)   // std::vector<std::pair<std::string /* field name */, multiplier> > _exchange_multipliers;
  { if ( (field_name == exchange_field_name) and mult.is_known(mv) )
      return !(mult.is_worked(mv, b, m));
  }
...
{ unsigned int q_points { 0 };
  ...
  { const auto& qp { _qso_points[static_cast<unsigned int>(m)] };

    for (const auto& b : score_bands)
      q_points += qp[static_cast<unsigned int>(b)];
  }
...
// UBA has weird rules
  ...
  { constexpr unsigned int ON_QSO_POINTS { 10 };  // hardwire the ON points for now
    ...
    for (const auto& m : score_modes)
    { const auto& qp { _n_ON_qsos[static_cast<unsigned int>(m)] };

      for (const auto& b : score_bands)
        bonus_qsos += qp[static_cast<unsigned int>(b)];
    }
...
// total QSOs
    ...
    for (const auto& m : score_modes)
    { for (const auto& b : score_bands)
      { total_qsos += _n_qsos[static_cast<unsigned int>(m)][static_cast<unsigned int>(b)];
        total_qsos -= _n_dupes[to_uint(m)][to_uint(b)];
      }
    }
...
*/
STRING_MAP<MULT_SET /* values */ > running_statistics::worked_exchange_mults(const BAND b, const MODE m) const   // key of return value = field name
...
// clear the mults
  FOR_ALL(_callsign_multipliers, [] (pair<const string, multiplier>& callsign_m) { callsign_m.second.clear(); } );  // so annoying that can't use [a, b] in lambda
  _country_multipliers.clear();
  FOR_ALL(_exchange_multipliers, [] (pair<string, multiplier>& sm) { sm.second.clear(); } );  // so annoying that can't use [a, b] in lambda
}
...
// current QSO points
  for (const auto& m : score_modes)
  { const auto& qp { _qso_points[to_uint(m)] };

    FOR_ALL(score_bands, [&current_qso_points, &qp] (const BAND& b) { current_qso_points += qp[to_int(b)]; } );
  }
...
// add a notional QSO; first calculate average qso point worth of a qso on the current band and mode
  const unsigned int& n_qso_points { _qso_points[to_uint(m)][to_uint(b)] };
  const unsigned int& n_qsos       { _n_qsos[to_uint(m)][to_uint(b)] };
...
{ const set<BAND>& score_bands { rules.score_bands() };
  ...
  for (const auto& m : score_modes)
  { const auto& nq { _n_qsos[to_uint(m)] };

//    FOR_ALL(score_bands, [&nq, &rv] (const BAND b) { rv += nq[static_cast<int>(b)]; } );
    FOR_ALL(score_bands, [&nq, &rv] (const BAND b) { rv += nq[to_uint(b)]; } );
  }
...
{ const vector<BAND>& permitted_bands { rules.permitted_bands() };
  ...
  for (const auto& [ _, mult ] : _callsign_multipliers)
  { if (mult.per_band())
      FOR_ALL(permitted_bands, [&mult, &rv, &score_bands] (const auto& b) { if (score_bands.contains(b)) rv += mult.n_worked(b); });
    else
...
{ unsigned int rv { 0 };
  ...
  for (const auto& [_, mults] : worked_exchange_mults(b, m))
    rv += mults.size();
...
*/
bool call_history::worked(const string_view s, const BAND b , const MODE m) const
{ SAFELOCK(_history);
...
*/
bool call_history::worked(const string_view s, const BAND b) const
{ SAFELOCK(_history);
...
*/
bool call_history::worked(const string_view s, const MODE m) const
{ SAFELOCK(_history);

  for (const auto& [call, bms] : _history)
  { if (call == s)
    { if (ANY_OF(bms, [m] (const bandmode& bm) { return (bm.second == m); }))
        return true;
    }
  }
...
*/
bool call_history::worked(const string_view s) const
{ SAFELOCK(_history);
...
*/
bool call_history::worked_on_another_band(const string_view s, const BAND b) const
{ SAFELOCK(_history);

  for (const auto& [call, bms] : _history)
  { if (call == s)
    { if (ANY_OF(bms, [b] (const bandmode& bm) { return (bm.first != b); }))
        return true;
    }
  }
...
*/
bool call_history::worked_on_another_mode(const string_view s, const MODE m) const
{ SAFELOCK(_history);
...
*/
bool call_history::worked_on_another_band_and_mode(const string_view s, const BAND b, const MODE m)
{ SAFELOCK(_history);
